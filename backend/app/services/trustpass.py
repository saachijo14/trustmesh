...
def issue_trustpass(
    ...
    Customer identity is passed explicitly because the policy result
    contains policy information, not customer identity.
    ...
    result = {
        ...
        "requires_human_approval": policy_result[
...
def _refresh_expiry_status(trustpass: dict) -> None:
    ...
    if (
        trustpass.get("status") == "active"
        and is_expired(trustpass)
    ):
...
if __name__ == "__main__":
    ...
    risk = compute_risk_score(customer_id)
    policy = apply_policy(
        risk,
        order_amount=15000,
    )
